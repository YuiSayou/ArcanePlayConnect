...
public class WebhookListenerService
{
    ...
    private WebhookEvent? ParseTikFinityPayload(string body, string contentType)
    {
        ...
        // ?? triggerTypeId ?????????????????????????????????????????????????
        // triggerTypeId=9  ? follow
        // triggerTypeId=3  ? gift
        // triggerTypeId=6  ? join
        ...
        var isJoin =
            triggerTypeId == 6 ||
            string.Equals(Get(f, "content"), "Join", StringComparison.OrdinalIgnoreCase) ||
            ContainsWord(Get(f, "type"), "join") ||
            ContainsWord(Get(f, "event"), "join") ||
            ContainsWord(Get(f, "action"), "join");

        if (isJoin)
        {
            evt.EventType = WebhookEventType.Join;
            _logger.LogInfo($"Join event - Nickname: {evt.Nickname}", LogCategory.Join);
            return evt;
        }
